...
import { Project, Chapter, Scene, StoryboardShot } from '../../../../shared/types';
...
export default function ScenesPage() {
    ...
    const updateChapters = (newChapters: Chapter[]) => {
        updateProjectState(prev => ({ ...prev, chapters: newChapters }));
    };

    const handleUpdateScene = (updates: Partial<Scene>) => {
        updateProjectState(prev => {
            if (!selectedChapterId || !selectedSceneId) return prev;
            return {
                ...prev,
    ...
    const handleUpdateShot = (shotId: string, updates: Partial<StoryboardShot>) => {
        updateProjectState(prev => {
            if (!selectedChapterId || !selectedSceneId) return prev;
            return {
                ...prev,
                        ...
                        return {
                            ...
                            scenes: c.scenes.map(s => {
                                if (s.id === selectedSceneId) {
                                    return {
                                        ...s,
                                        storyboard: (s.storyboard || []).map(shot =>
